...
const TABS_META = [
  { id: "accueil", cle: "entretiens" },
  { id: "corpus", cle: "corpus" },
  { id: "glossaire", cle: "glossaire" },
  { id: "reglages", cle: "reglages" },
];
  ...
  return (
    <nav className="tabbar" aria-label="Navigation principale">
      {TABS_META.map((tab) => {
        ...
        return (
          <button key={tab.id} className={"tab" + (a ? " tab-actif" : "")}
            onClick={() => onChange(tab.id)} aria-current={a ? "page" : undefined}>
            <span className="tab-icon">{ICONES[tab.id](a)}</span>
            <span className="tab-label">{t(`tabbar.${tab.cle}`)}</span>
