...
- **Suite de tests automatisés (39 tests, exécutés à chaque poussée sur main)** : jusqu'ici, chaque correctif de cette conversation était vérifié manuellement puis jeté — aucune protection contre une régression future. `backend/tests/` couvre désormais en priorité les zones les plus sujettes aux bugs réels rencontrés pendant le développement : le moteur statistique de base et avancé (alpha de Cronbach, AFE, **AFC avec des noms de construits réalistes — régression du bug de sanitisation des identifiants semopy**, régression multiple, **médiation avec des variables à échelles très différentes — régression du bug de standardisation**), la réparation automatique des codes de questionnaire dupliqués (**avec vérification que la correction persiste réellement en base, pas seulement en mémoire — régression du bug SQLAlchemy `is_modified()`**), le système de crédits (déduction, remboursement automatique sur échec, admin jamais facturé), l'idempotence des paiements Google Play (un jeton d'achat rejoué ne crédite jamais deux fois), et l'authentification. Intégrée à la CI GitHub Actions (`backend-ci.yml`) : toute poussée sur `main` qui casse un de ces comportements est bloquée avant même d'atteindre la vérification Docker.
- **Diagnostics économétriques et suppression de corpus** : les régressions d'une étude en branche sciences économiques intègrent désormais le VIF (multicolinéarité), le test de Breusch-Pagan (hétéroscédasticité), le test de Jarque-Bera (normalité des résidus) et les élasticités au point moyen — validées avec des données aux propriétés connues à l'avance (colinéarité construite détectée à VIF=266, élasticité estimée à 0,294 contre 0,285 théorique). Absents en branche sciences humaines (non-régression testée). La suppression de corpus, absente jusqu'ici, préserve les entretiens rattachés (jamais supprimés, simplement détachés).
- **Sélection des analyses par le chercheur** : au moment d'importer les données, le chercheur choisit désormais précisément quelles analyses avancées inclure (AFE, AFC, régressions, médiation, passerelle qualitative-quantitative, synthèse interprétative IA) — les statistiques descriptives, la fiabilité et les corrélations restent toujours calculées, socle non désactivable. Celui qui veut se limiter aux corrélations n'est plus obligé de subir une AFC ou une médiation qu'il n'a jamais demandées. Comportement historique (tout activé) intégralement préservé quand aucune préférence n'est transmise, testé explicitement.
...
# Suite de tests automatisés (39 tests, ~7 secondes)
